...
class App extends Component {
  ...
  onClickImage = thumbnailUrl => {
    ...
    const findImageList = imagesList.find(
      each => each.thumbnailUrl === thumbnailUrl,
    )
    const {imageUrl} = findImageList
    if (imageUrl === imgUrl) {
      const newImgUrl =
  ...
  render() {
    const {score, timer, imgUrl, category, isTrue} = this.state
      ...
      <div className="bg-container">
        ...
        {!isTrue && (
          <div className="card-container">
            <img src={imgUrl} className="image" alt="match" />
